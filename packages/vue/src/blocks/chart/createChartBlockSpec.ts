import { createVueBlockSpec, type SlashMenuConfig, type VueBlockSpec } from '../createVueBlockSpec';
...
export function createChartBlockSpec(options: ChartBlockSpecOptions = {}): VueBlockSpec<{
  data: { default: ChartNodeData };
}> {
  const defaultType = options.defaultType ?? 'bar';

  return createVueBlockSpec(
    {
      type: 'chart',
      propSchema: {
        data: {
...
export type { ChartBlockType, ChartNodeData } from './chartTypes';
export {
